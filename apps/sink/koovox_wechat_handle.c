...
void koovox_handle_wechat_indicate(uint16 cid, uint16 handle)
{
	...
	{
		uint16* message = (uint16*)mallocPanic(sizeof(uint16));
	
		*message = cid;
		
		/* 排队等待 */
		MessageSendLater(&theSink.task, EventKoovoxRepeatSendIndication, message, 1000);
	}
}
...
void koovox_indicate_to_wechat(uint16 cid, uint16 handle)
{
	if(g_send_data.len > g_send_data.offset)
	{
		...
		GattIndicationRequest(&theSink.rundata->gatt.task, cid, handle, len_send,g_send_data.data + g_send_data.offset);
